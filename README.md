<h1 align="center">Hi 👋, I'm Bidhan Singh</h1>
<h3 align="center">Flutter Developer | Mobile App Enthusiast</h3>
![Uploading banner_my.jpeg…]()


---

## 🚀 About Me
- 💻 Flutter Developer (Dart)
- 🔥 Building real-world mobile apps
- 🌱 Currently learning Backend (PHP, MySQL, APIs)
- 🏢 Intern at Octopyder Services Pvt. Ltd.
- 🎯 Goal: Become a full-stack mobile app developer

---

## 🛠 Tech Skills
- Flutter & Dart  
- Firebase  
- REST API Integration  
- Git & GitHub  
- PHP & MySQL (Backend learning)  
- UI/UX Design (Mobile Apps)

---

## 📂 My Projects
🔹 **Institute Manager Application**  
A Flutter app for institute management with role-based features.

🔹 **Flutter UI Projects**  
Modern mobile UI designs using Flutter.

---

## 📫 Contact Me
- GitHub: https://github.com/Bidhansingh9931  
- Email: bidhansingh9931@gmail.com  

---

⭐ *If you like my projects, consider starring them!*
